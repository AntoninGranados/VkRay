...
Field Field::makeNumeric(FieldType type, std::string id, std::string label, const std::vector<float>& n, NumericMeta metadata, bool animatable) {
    switch (type) {
        ...
        case FieldType::Vec2:  return Field::make<glm::vec2>(std::move(id), std::move(label), n.size() == 1 ? glm::vec2(n[0]) : glm::vec2(n[0], n[1]), metadata, animatable);
        case FieldType::Vec3:  return Field::make<glm::vec3>(std::move(id), std::move(label), n.size() == 1 ? glm::vec3(n[0]) : glm::vec3(n[0], n[1], n[2]), metadata, animatable);
        case FieldType::Vec4:  return Field::make<glm::vec4>(std::move(id), std::move(label), n.size() == 1 ? glm::vec4(n[0]) : glm::vec4(n[0], n[1], n[2], n[3]), metadata, animatable);
        case FieldType::IVec2: return Field::make<glm::ivec2>(std::move(id), std::move(label), n.size() == 1 ? glm::ivec2(int(n[0])) : glm::ivec2(int(n[0]), int(n[1])), metadata, animatable);
        case FieldType::IVec3: return Field::make<glm::ivec3>(std::move(id), std::move(label), n.size() == 1 ? glm::ivec3(int(n[0])) : glm::ivec3(int(n[0]), int(n[1]), int(n[2])), metadata, animatable);
        default:               return Field::make<glm::ivec4>(std::move(id), std::move(label), n.size() == 1 ? glm::ivec4(int(n[0])) : glm::ivec4(int(n[0]), int(n[1]), int(n[2]), int(n[3])), metadata, animatable);
    }
}
