...
std::string ProgrammableShader::assignParam(const ecs::ComponentField& field, int& offset) const {
    ...
    for (int i = 0; i < spec.components; i++) {
        if (i > 0) args += ", ";
        std::string value = std::format("programmableParams.values[base+{}]", offset + i);
        if (spec.isInt) {
...
void ProgrammableShader::load(bool migrate) {
    ...
    if (!GlslDslFile::parse(path, { .manglePrefix = mangledPrefix(), .expectedVersion = static_cast<int>(SHADER_VERSION) }))
        return;
...
size_t ProgrammableShader::capacityFromCount(size_t count) {
    size_t cap = 16;
    while (cap < count) cap <<= 1;
    return cap;
}

void ProgrammableShader::packAll(const FrameContext& frame) {
    std::vector<float> values;
    for (ProgrammableShader* shader : Scene::getProgrammableShaders()) {
        shader->baseOffset = static_cast<int>(values.size());
        const std::vector<float> packed = shader->packValues();
        values.insert(values.end(), packed.begin(), packed.end());
    }

    static size_t capacity = 16;
    const BufferHandle handle = PathtraceRenderer::getProgrammableParamsHandle();
    const size_t required = capacityFromCount(values.size());
    if (required > capacity) {
        Core::getEngine().resizeBuffer(handle, required * sizeof(float));
        capacity = required;
    }

    Buffer& buffer = Core::getEngine().getBuffer(handle, frame.currentFrame);
    values.resize(buffer.getSize() / sizeof(float));
    Core::getEngine().fillBuffer(buffer, values.data());
}
...
void ProgrammableShader::generateDispatch() {
    ...
    for (ProgrammableShader* shader : Scene::getProgrammableShaders()) {
        ...
        functions += std::format(
            ...
            "{}"
            "{}"
            "Material {} (int base, vec3 pos, vec2 uv, vec3 normal, vec3 wo, RngState rng, inout vec3 new_normal) {{\n"
            "{}"
            "Material mat;\n"
            "{}"
            "return mat;\n"
    ...
    std::string content = std::format(
        "{}\n"
        "Material dispatchProgrammable(in Material mat, inout Hit hit, in vec3 wo, inout RngState rng) {{\n"
        "    vec3 pos    = hit.p;\n"
        "    vec3 normal = hit.normal;\n"
        "    vec2 uv     = hit.uv;\n"
        "    vec3 new_normal = normal;\n"
        "    int base = int(mat.payload[1]);\n"
        "    Material result = DEFAULT_MATERIAL;\n"
        "    switch (int(mat.payload[0])) {{\n"
        "{}"
        "    }}\n"
